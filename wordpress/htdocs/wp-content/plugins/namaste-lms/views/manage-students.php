<div class="wrap">
   ...
   <?php _e('Filter by user login:', 'namaste');?> <input type="text" name="user_login" value="<?php echo esc_attr(@$_GET['user_login'])?>"><br />
	...
	<?php if(!empty($_GET['course_id'])):?>
		...
		<table class="widefat">
			<tr>
				...
				<?php endif;?>
				<th><a href="admin.php?page=namaste_students&course_id=<?php echo intval($_GET['course_id'])?>&status=<?php echo esc_attr($_GET['status'])?>&user_login=<?php echo empty($_GET['user_login']) ? '' : esc_attr($_GET['user_login'])?>&user_email=<?php echo empty($_GET['user_email']) ? '' : esc_attr($_GET['user_email'])?>&ob=display_name&dir=<?php echo $odir?>&page_limit=<?php echo $page_limit;?>&filter_tags=<?php echo empty($_GET['filter_tags']) ? '' : esc_attr($_GET['filter_tags']);?>"><?php _e('Student', 'namaste')?></a></th>
