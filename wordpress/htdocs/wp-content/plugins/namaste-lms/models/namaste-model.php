...
class NamasteLMS {
   static function install($update = false) {
   	...
   	update_option( 'namaste_version', "1.47");
   	if(!$update) self::init();
   ...
   static function menu() {   	  	
	...
	static function options() {
		...
		if(!empty($_POST['namaste_options']) and check_admin_referer('save_options', 'nonce_options')) {
			...
			// login texts
			update_option('namaste_need_login_text_lesson', $_POST['need_login_text_lesson']);
			update_option('namaste_need_login_text_course', $_POST['need_login_text_course']);
	...
	static function erase_data($email_address, $page = 1) {
		 ...
		 $number = 200; // Limit us to avoid timing out
  		 ...
  		 if($page == 1) {
			 ...
			 $wpdb->query($wpdb->prepare("DELETE FROM ".NAMASTE_STUDENT_COURSES." WHERE student_id=%d", $user->ID));
