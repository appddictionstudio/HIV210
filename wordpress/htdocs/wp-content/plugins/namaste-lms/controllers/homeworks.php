	...
	static function submit_solution($in_shortcode = false) {
		...
		if(!empty($_POST['ok'])) {
			...
			if(!$exists) {
				...
				// print_r($_SESSION['namaste_file_errors']);
				// these lines are in case of using Ajax uploads. Then we have stored the errors in session
				if(empty($total_size_error) and !empty($_SESSION['namaste_file_errors'][0])) $total_size_error = $_SESSION['namaste_file_errors'][0];
				if(empty($file_errors) and !empty($_SESSION['namaste_file_errors'][1])) $file_errors = $_SESSION['namaste_file_errors'][1];
				if(empty($file_size_errors) and !empty($_SESSION['namaste_file_errors'][2])) $file_size_errors = $_SESSION['namaste_file_errors'][2];
				if(empty($file_not_uploaded_errors) and !empty($_SESSION['namaste_file_errors'][3])) $file_not_uploaded_errors = $_SESSION['namaste_file_errors'][3];
			...
			if(isset($_SESSION['namaste_file_errors'])) unset($_SESSION['namaste_file_errors']);
	...
	static function upload_files($homework = null, $solution_id = 0) {
		...
		if(intval($total_size_limit) > 0) {
			...
			if($total_size > $total_size_limit) {
				$_SESSION['namaste_file_errors'] = array($total_size, null, null, null); // return total size error
				return $_SESSION['namaste_file_errors'];
			}
		} // end checking size
		...
		$_SESSION['namaste_file_errors'] = array($total_size_error, $file_errors, $file_size_errors, $file_not_uploaded_errors);		
